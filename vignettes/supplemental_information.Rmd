...
probes and teh `prcomp` from the `stats` R package. We condensed autosomal DNAm across 
35,360 samples into 1,000 hashed feature columns, then used this reduced representation 
(1,000 rows X 35,360 columns) in cluster analysis. Feature hashing, or the 
"hashing trick" (@weinberger_feature_2010), was used as an intermediate dimensionality 
reduction step. The data files for each performed test are available at https://recount.bio/data/recountmethylation_manuscript_supplement/data/. 

To perform feature hashing, we used the following script in Python 3, where `target_dim=1000` 
specifies the target hashed feature dimension:
