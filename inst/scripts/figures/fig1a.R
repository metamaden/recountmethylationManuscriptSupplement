...
# Author: Sean Maden
# Make Figure 1A panel plot of cumulative GEO samples (GSMs) by year and 
...
vv <- c("green", "forestgreen", "blue", "cyan", "brown", "gold")
...
fig1a <- ggplot(dff, aes(x = year, y = samples)) + theme_bw() +
...
# get the plot legend
fig1a.lform <- ggplot(dff, aes(x = year, y = samples)) + theme_bw() +
  xlab("Year") + ylab("Cumulative samples") + xlim(2009, 2018) +
